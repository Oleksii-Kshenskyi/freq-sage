...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        cli::Commands::Train { file } => {
            ...
            let new_freqs = db
                .freqs_of_words(&data.freqs.keys().cloned().collect::<Vec<String>>())
                .context("main(): while trying to re-retrieve new values of frequencies.")?;

            db.insert_rankings(new_freqs, data.sentences)
